...
<head>
    ...
    <title>
        <?php echo htmlspecialchars($target_user['full_name']) ?> – WildMatch
    </title>
    ...
    <main class="container">
        ...
        <section class="info-section">
            ...
            <p><strong>Stad:</strong>
                <?php echo htmlspecialchars($target_user['city'] ?: 'Inte angivet') ?>
            </p>
            <p><strong>Preferens:</strong>
                <?php echo htmlspecialchars($target_user['preference']) ?>
            </p>
            <p><strong>Medlem sedan:</strong>
                <?php echo date('j F Y', strtotime($target_user['created_at'])) ?>
            </p>

            <?php if (! empty($target_user['bio'])): ?>
            <p><strong>Om mig:</strong><br>
                <em>"
                    <?php echo htmlspecialchars($target_user['bio']) ?>"
                </em>
            </p>
            <?php endif; ?>
            ...
            <p><strong>Årslön:</strong>
                <?php echo number_format($target_user['salary'], 0, ',', ' ') ?> €
            </p>
            <?php endif; ?>
            <?php endif; ?>
        </section>
        ...
        <div style="margin: 1rem 0;">
            <button id="like-btn" data-action="<?php echo $is_liked ? 'unlike' : 'like' ?>" class="book-btn">
                <?php echo $is_liked ? 'Gilla inte längre' : 'Gilla denna profil' ?>
            </button>
        ...
        <section class="info-section">
            ...
            <form method="POST" action="comment.php" style="margin-top: 1rem;">
                <input type="hidden" name="target_id" value="<?php echo (int)$user_id ?>">
                <textarea name="content" placeholder="Skriv en kommentar..." required rows="3"
