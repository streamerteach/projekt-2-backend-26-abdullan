...
<main class="container">
    ...
    <form method="POST" class="info-section">
        <p><label>Namn/Rubrik: <input type="text" name="full_name" value="<?= htmlspecialchars($user['full_name']) ?>" required></label></p>
        <p><label>E-post: <input type="email" name="email" value="<?= htmlspecialchars($user['email']) ?>" required></label></p>
        <p><label>Stad: <input type="text" name="city" value="<?= htmlspecialchars($user['city']) ?>"></label></p>
        <p><label>Årslön: <input type="number" name="salary" value="<?= (int)$user['salary'] ?>" min="0"></label></p>
        <p>
            <label>Preferens:
        ...
        <p><textarea name="bio" placeholder="Berätta om dig..." rows="4" style="width:100%;"><?= htmlspecialchars($user['bio']) ?></textarea></p>
